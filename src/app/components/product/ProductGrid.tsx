...
const ProductGrid: React.FC<Props> = ({ products, onChangeStatus, onDelete }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
      {products.map((p) => (
        <div key={p.Id} className="rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          {/* Header */}
          ...
          <div className="p-4 space-y-3 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700">{p.FabricType} — {p.FabricName} · {p.GSM} GSM</span>
            </div>
            <div className="text-slate-500">
              Created: <span className="text-gray-700">{formatDate(p.CreatedOn)}</span>
            </div>
            {p.Description && (
